...
public class Iregister : ReportClass
{
  ...
  {
    lock (Iregister.__ENCList)
    {
      if (Iregister.__ENCList.Count == Iregister.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (Iregister.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (Iregister.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              Iregister.__ENCList[index1] = Iregister.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        Iregister.__ENCList.RemoveRange(index1, checked (Iregister.__ENCList.Count - index1));
        Iregister.__ENCList.Capacity = Iregister.__ENCList.Count;
      }
      Iregister.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }

  public Iregister() => Iregister.__ENCAddToList((object) this);
  ...
  {
    get => "MauliModule7.Iregister.rpt";
    set
    {
    }
  }
