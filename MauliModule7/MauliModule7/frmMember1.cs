...
public class frmMember1 : Form
{
  ...
  public frmMember1()
  {
    this.KeyDown += new KeyEventHandler(this.frmMember1_KeyDown);
    this.Load += new EventHandler(this.frmMember_Load);
    frmMember1.__ENCAddToList((object) this);
    this.SearchType = 7;
  ...
  {
    lock (frmMember1.__ENCList)
    {
      if (frmMember1.__ENCList.Count == frmMember1.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (frmMember1.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (frmMember1.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              frmMember1.__ENCList[index1] = frmMember1.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        frmMember1.__ENCList.RemoveRange(index1, checked (frmMember1.__ENCList.Count - index1));
        frmMember1.__ENCList.Capacity = frmMember1.__ENCList.Count;
      }
      frmMember1.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }
  ...
  private void InitializeComponent()
  {
    ...
    ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof (frmMember1));
    ...
    Point point1 = new Point(289, 530);
    Point point2 = point1;
    ...
    point1 = new Point(257, 16 /*0x10*/);
    Point point11 = point1;
    label1_1.Location = point11;
    this.Label1.Name = "Label1";
    Label label1_2 = this.Label1;
    size1 = new Size(663, 15);
    Size size11 = size1;
    label1_2.Size = size11;
    this.Label1.TabIndex = 20;
    this.Label1.Text = "Tab - Next Field  ||  Shift + Tab - Prev. Field    ||    F3 - Namewise List   ||   F4 - CodeWise List    ||    Ctrl + Tab - Exit from Grid";
  ...
  private void frmMember1_KeyDown(object sender, KeyEventArgs e)
  {
  ...
  public DataSet GetFillGrid()
  {
    ...
    else
    {
      ...
      while (index1 <= num)
      {
        if (Strings.InStr(Strings.UCase(Strings.Trim(this.txtSearchbox.Text)), Strings.UCase(Strings.Mid(MyProject.Forms.frmConstantGrid.DataGridViewMember.Rows[index1].Cells[7].Value.ToString().Trim(), 1, Strings.Len(this.txtSearchbox.Text)))) == 1)
        {
          this.DataGridView2.Rows.Add();
          int index2;
