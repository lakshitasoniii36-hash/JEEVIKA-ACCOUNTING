...
// Type: MauliModule7.CReportBillRegister
...
public class CReportBillRegister : ReportClass
{
  private static List<WeakReference> __ENCList = new List<WeakReference>();

  [DebuggerNonUserCode]
  static CReportBillRegister()
  {
  }
  ...
  {
    lock (CReportBillRegister.__ENCList)
    {
      if (CReportBillRegister.__ENCList.Count == CReportBillRegister.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (CReportBillRegister.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (CReportBillRegister.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              CReportBillRegister.__ENCList[index1] = CReportBillRegister.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        CReportBillRegister.__ENCList.RemoveRange(index1, checked (CReportBillRegister.__ENCList.Count - index1));
        CReportBillRegister.__ENCList.Capacity = CReportBillRegister.__ENCList.Count;
      }
      CReportBillRegister.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }

  public CReportBillRegister() => CReportBillRegister.__ENCAddToList((object) this);

  public override string ResourceName
  {
    get => "CReportBillRegister.rpt";
    set
    {
    }
  }
  ...
  {
    get => "MauliModule7.CReportBillRegister.rpt";
    set
    {
    }
  }
