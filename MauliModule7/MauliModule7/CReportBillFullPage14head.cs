...
// Type: MauliModule7.CReportBillFullPage14head
...
public class CReportBillFullPage14head : ReportClass
{
  private static List<WeakReference> __ENCList = new List<WeakReference>();

  [DebuggerNonUserCode]
  static CReportBillFullPage14head()
  {
  }
  ...
  {
    lock (CReportBillFullPage14head.__ENCList)
    {
      if (CReportBillFullPage14head.__ENCList.Count == CReportBillFullPage14head.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (CReportBillFullPage14head.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (CReportBillFullPage14head.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              CReportBillFullPage14head.__ENCList[index1] = CReportBillFullPage14head.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        CReportBillFullPage14head.__ENCList.RemoveRange(index1, checked (CReportBillFullPage14head.__ENCList.Count - index1));
        CReportBillFullPage14head.__ENCList.Capacity = CReportBillFullPage14head.__ENCList.Count;
      }
      CReportBillFullPage14head.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }

  public CReportBillFullPage14head() => CReportBillFullPage14head.__ENCAddToList((object) this);

  public override string ResourceName
  {
    get => "CReportBillFullPage14head.rpt";
    set
    {
    }
  }
  ...
  {
    get => "MauliModule7.CReportBillFullPage14head.rpt";
    set
    {
    }
  }
