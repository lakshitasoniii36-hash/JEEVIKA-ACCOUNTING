...
// Type: MauliModule7.CReportMemberOSList14head
...
public class CReportMemberOSList14head : ReportClass
{
  private static List<WeakReference> __ENCList = new List<WeakReference>();

  [DebuggerNonUserCode]
  static CReportMemberOSList14head()
  {
  }
  ...
  {
    lock (CReportMemberOSList14head.__ENCList)
    {
      if (CReportMemberOSList14head.__ENCList.Count == CReportMemberOSList14head.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (CReportMemberOSList14head.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (CReportMemberOSList14head.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              CReportMemberOSList14head.__ENCList[index1] = CReportMemberOSList14head.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        CReportMemberOSList14head.__ENCList.RemoveRange(index1, checked (CReportMemberOSList14head.__ENCList.Count - index1));
        CReportMemberOSList14head.__ENCList.Capacity = CReportMemberOSList14head.__ENCList.Count;
      }
      CReportMemberOSList14head.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }

  public CReportMemberOSList14head() => CReportMemberOSList14head.__ENCAddToList((object) this);

  public override string ResourceName
  {
    get => "CReportMemberOSList14head.rpt";
    set
    {
    }
  }
  ...
  {
    get => "MauliModule7.CReportMemberOSList14head.rpt";
    set
    {
    }
  }
