...
// Type: MauliModule7.CReportBillFullPageWithReceipt21HeadNew
...
public class CReportBillFullPageWithReceipt21HeadNew : ReportClass
{
  private static List<WeakReference> __ENCList = new List<WeakReference>();

  [DebuggerNonUserCode]
  static CReportBillFullPageWithReceipt21HeadNew()
  {
  }
  ...
  {
    lock (CReportBillFullPageWithReceipt21HeadNew.__ENCList)
    {
      if (CReportBillFullPageWithReceipt21HeadNew.__ENCList.Count == CReportBillFullPageWithReceipt21HeadNew.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (CReportBillFullPageWithReceipt21HeadNew.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (CReportBillFullPageWithReceipt21HeadNew.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              CReportBillFullPageWithReceipt21HeadNew.__ENCList[index1] = CReportBillFullPageWithReceipt21HeadNew.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        CReportBillFullPageWithReceipt21HeadNew.__ENCList.RemoveRange(index1, checked (CReportBillFullPageWithReceipt21HeadNew.__ENCList.Count - index1));
        CReportBillFullPageWithReceipt21HeadNew.__ENCList.Capacity = CReportBillFullPageWithReceipt21HeadNew.__ENCList.Count;
      }
      CReportBillFullPageWithReceipt21HeadNew.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }

  public CReportBillFullPageWithReceipt21HeadNew()
  {
    CReportBillFullPageWithReceipt21HeadNew.__ENCAddToList((object) this);
  }

  public override string ResourceName
  {
    get => "CReportBillFullPageWithReceipt21HeadNew.rpt";
    set
    {
    }
  }
  ...
  {
    get => "MauliModule7.CReportBillFullPageWithReceipt21HeadNew.rpt";
    set
    {
    }
  }
