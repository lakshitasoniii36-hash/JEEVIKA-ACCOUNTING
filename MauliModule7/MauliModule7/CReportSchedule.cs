...
// Type: MauliModule7.CReportSchedule
...
public class CReportSchedule : ReportClass
{
  private static List<WeakReference> __ENCList = new List<WeakReference>();

  [DebuggerNonUserCode]
  static CReportSchedule()
  {
  }
  ...
  {
    lock (CReportSchedule.__ENCList)
    {
      if (CReportSchedule.__ENCList.Count == CReportSchedule.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (CReportSchedule.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (CReportSchedule.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              CReportSchedule.__ENCList[index1] = CReportSchedule.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        CReportSchedule.__ENCList.RemoveRange(index1, checked (CReportSchedule.__ENCList.Count - index1));
        CReportSchedule.__ENCList.Capacity = CReportSchedule.__ENCList.Count;
      }
      CReportSchedule.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }

  public CReportSchedule() => CReportSchedule.__ENCAddToList((object) this);

  public override string ResourceName
  {
    get => "CReportSchedule.rpt";
    set
    {
    }
  }
  ...
  {
    get => "MauliModule7.CReportSchedule.rpt";
    set
    {
    }
  }
  ...
  [Browsable(false)]
  [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
  public IParameterField Parameter_PPrevYear
  {
