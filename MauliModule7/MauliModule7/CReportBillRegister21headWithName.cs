...
// Type: MauliModule7.CReportBillRegister21headWithName
...
public class CReportBillRegister21headWithName : ReportClass
{
  private static List<WeakReference> __ENCList = new List<WeakReference>();

  [DebuggerNonUserCode]
  static CReportBillRegister21headWithName()
  {
  }
  ...
  {
    lock (CReportBillRegister21headWithName.__ENCList)
    {
      if (CReportBillRegister21headWithName.__ENCList.Count == CReportBillRegister21headWithName.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (CReportBillRegister21headWithName.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (CReportBillRegister21headWithName.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              CReportBillRegister21headWithName.__ENCList[index1] = CReportBillRegister21headWithName.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        CReportBillRegister21headWithName.__ENCList.RemoveRange(index1, checked (CReportBillRegister21headWithName.__ENCList.Count - index1));
        CReportBillRegister21headWithName.__ENCList.Capacity = CReportBillRegister21headWithName.__ENCList.Count;
      }
      CReportBillRegister21headWithName.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }

  public CReportBillRegister21headWithName()
  {
    CReportBillRegister21headWithName.__ENCAddToList((object) this);
  }

  public override string ResourceName
  {
    get => "CReportBillRegister21headWithName.rpt";
    set
    {
    }
  }
  ...
  {
    get => "MauliModule7.CReportBillRegister21headWithName.rpt";
    set
    {
    }
  }
  ...
  [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
  [Browsable(false)]
  public Section ReportFooterSection2 => this.ReportDefinition.Sections[11];

  [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
  [Browsable(false)]
  public Section ReportFooterSection1 => this.ReportDefinition.Sections[12];

  [Browsable(false)]
  [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
  public Section ReportFooterSection3 => this.ReportDefinition.Sections[13];
  ...
  [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
  [Browsable(false)]
  public IParameterField Parameter_vArea
  {
