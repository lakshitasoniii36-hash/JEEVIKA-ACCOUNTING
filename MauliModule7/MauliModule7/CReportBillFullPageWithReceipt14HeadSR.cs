...
// Type: MauliModule7.CReportBillFullPageWithReceipt14HeadSR
...
public class CReportBillFullPageWithReceipt14HeadSR : ReportClass
{
  private static List<WeakReference> __ENCList = new List<WeakReference>();

  [DebuggerNonUserCode]
  static CReportBillFullPageWithReceipt14HeadSR()
  {
  }
  ...
  {
    lock (CReportBillFullPageWithReceipt14HeadSR.__ENCList)
    {
      if (CReportBillFullPageWithReceipt14HeadSR.__ENCList.Count == CReportBillFullPageWithReceipt14HeadSR.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (CReportBillFullPageWithReceipt14HeadSR.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (CReportBillFullPageWithReceipt14HeadSR.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              CReportBillFullPageWithReceipt14HeadSR.__ENCList[index1] = CReportBillFullPageWithReceipt14HeadSR.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        CReportBillFullPageWithReceipt14HeadSR.__ENCList.RemoveRange(index1, checked (CReportBillFullPageWithReceipt14HeadSR.__ENCList.Count - index1));
        CReportBillFullPageWithReceipt14HeadSR.__ENCList.Capacity = CReportBillFullPageWithReceipt14HeadSR.__ENCList.Count;
      }
      CReportBillFullPageWithReceipt14HeadSR.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }

  public CReportBillFullPageWithReceipt14HeadSR()
  {
    CReportBillFullPageWithReceipt14HeadSR.__ENCAddToList((object) this);
  }

  public override string ResourceName
  {
    get => "CReportBillFullPageWithReceipt14HeadSR.rpt";
    set
    {
    }
  }
  ...
  {
    get => "MauliModule7.CReportBillFullPageWithReceipt14HeadSR.rpt";
    set
    {
    }
  }
