...
// Type: MauliModule7.CReportBillBifDouble
...
public class CReportBillBifDouble : ReportClass
{
  private static List<WeakReference> __ENCList = new List<WeakReference>();

  [DebuggerNonUserCode]
  static CReportBillBifDouble()
  {
  }
  ...
  {
    lock (CReportBillBifDouble.__ENCList)
    {
      if (CReportBillBifDouble.__ENCList.Count == CReportBillBifDouble.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (CReportBillBifDouble.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (CReportBillBifDouble.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              CReportBillBifDouble.__ENCList[index1] = CReportBillBifDouble.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        CReportBillBifDouble.__ENCList.RemoveRange(index1, checked (CReportBillBifDouble.__ENCList.Count - index1));
        CReportBillBifDouble.__ENCList.Capacity = CReportBillBifDouble.__ENCList.Count;
      }
      CReportBillBifDouble.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }

  public CReportBillBifDouble() => CReportBillBifDouble.__ENCAddToList((object) this);

  public override string ResourceName
  {
    get => "CReportBillBifDouble.rpt";
    set
    {
    }
  }
  ...
  {
    get => "MauliModule7.CReportBillBifDouble.rpt";
    set
    {
    }
  }
