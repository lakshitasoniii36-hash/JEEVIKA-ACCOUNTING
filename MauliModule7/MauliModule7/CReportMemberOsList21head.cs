...
// Type: MauliModule7.CReportMemberOsList21head
...
public class CReportMemberOsList21head : ReportClass
{
  private static List<WeakReference> __ENCList = new List<WeakReference>();

  [DebuggerNonUserCode]
  static CReportMemberOsList21head()
  {
  }
  ...
  {
    lock (CReportMemberOsList21head.__ENCList)
    {
      if (CReportMemberOsList21head.__ENCList.Count == CReportMemberOsList21head.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (CReportMemberOsList21head.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (CReportMemberOsList21head.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              CReportMemberOsList21head.__ENCList[index1] = CReportMemberOsList21head.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        CReportMemberOsList21head.__ENCList.RemoveRange(index1, checked (CReportMemberOsList21head.__ENCList.Count - index1));
        CReportMemberOsList21head.__ENCList.Capacity = CReportMemberOsList21head.__ENCList.Count;
      }
      CReportMemberOsList21head.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }

  public CReportMemberOsList21head() => CReportMemberOsList21head.__ENCAddToList((object) this);

  public override string ResourceName
  {
    get => "CReportMemberOsList21head.rpt";
    set
    {
    }
  }
  ...
  {
    get => "MauliModule7.CReportMemberOsList21head.rpt";
    set
    {
    }
  }
  ...
  [Browsable(false)]
  [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
  public Section ReportFooterSection1 => this.ReportDefinition.Sections[8];
