...
// Type: MauliModule7.frmDigMemberSelection1
...
[DesignerGenerated]
public class frmDigMemberSelection1 : Form
{
  ...
  [DebuggerNonUserCode]
  static frmDigMemberSelection1()
  {
  }

  [DebuggerNonUserCode]
  public frmDigMemberSelection1()
  {
    this.Load += new EventHandler(this.frmDigMemberSelection1_Load);
    frmDigMemberSelection1.__ENCAddToList((object) this);
  ...
  {
    lock (frmDigMemberSelection1.__ENCList)
    {
      if (frmDigMemberSelection1.__ENCList.Count == frmDigMemberSelection1.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (frmDigMemberSelection1.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (frmDigMemberSelection1.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              frmDigMemberSelection1.__ENCList[index1] = frmDigMemberSelection1.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        frmDigMemberSelection1.__ENCList.RemoveRange(index1, checked (frmDigMemberSelection1.__ENCList.Count - index1));
        frmDigMemberSelection1.__ENCList.Capacity = frmDigMemberSelection1.__ENCList.Count;
      }
      frmDigMemberSelection1.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }
  ...
  private void InitializeComponent()
  {
    ...
    this.Name = nameof (frmDigMemberSelection1);
  ...
  private void btnListFrom_Click(object sender, EventArgs e)
  {
    ...
    {
      this.lblMemFrom.Text = Conversions.ToString(MyProject.Forms.frmMember1.DataGridView2.CurrentRow.Cells[1].Value);
      this.lblMemNameFrom.Text = Conversions.ToString(MyProject.Forms.frmMember1.DataGridView2.CurrentRow.Cells[6].Value);
    }
  ...
  private void btnListTo_Click(object sender, EventArgs e)
  {
    ...
    if (Constant.SearchStat & MyProject.Forms.frmMember1.DataGridView2.SelectedRows.Count == 1)
    {
      this.lblMemTo.Text = Conversions.ToString(MyProject.Forms.frmMember1.DataGridView2.CurrentRow.Cells[1].Value);
      this.lblMemNameTo.Text = Conversions.ToString(MyProject.Forms.frmMember1.DataGridView2.CurrentRow.Cells[6].Value);
    }
    Constant.SearchStat = false;
    Constant.SearchStr = "";
  }

  private void frmDigMemberSelection1_Load(object sender, EventArgs e)
  {
