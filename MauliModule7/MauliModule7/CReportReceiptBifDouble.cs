...
// Type: MauliModule7.CReportReceiptBifDouble
...
public class CReportReceiptBifDouble : ReportClass
{
  private static List<WeakReference> __ENCList = new List<WeakReference>();

  [DebuggerNonUserCode]
  static CReportReceiptBifDouble()
  {
  }
  ...
  {
    lock (CReportReceiptBifDouble.__ENCList)
    {
      if (CReportReceiptBifDouble.__ENCList.Count == CReportReceiptBifDouble.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (CReportReceiptBifDouble.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (CReportReceiptBifDouble.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              CReportReceiptBifDouble.__ENCList[index1] = CReportReceiptBifDouble.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        CReportReceiptBifDouble.__ENCList.RemoveRange(index1, checked (CReportReceiptBifDouble.__ENCList.Count - index1));
        CReportReceiptBifDouble.__ENCList.Capacity = CReportReceiptBifDouble.__ENCList.Count;
      }
      CReportReceiptBifDouble.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }

  public CReportReceiptBifDouble() => CReportReceiptBifDouble.__ENCAddToList((object) this);

  public override string ResourceName
  {
    get => "CReportReceiptBifDouble.rpt";
    set
    {
    }
  }
  ...
  {
    get => "MauliModule7.CReportReceiptBifDouble.rpt";
    set
    {
    }
  }
