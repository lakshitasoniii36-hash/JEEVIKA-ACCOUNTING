...
// Type: MauliModule7.CReportBillFullPage21head
...
public class CReportBillFullPage21head : ReportClass
{
  private static List<WeakReference> __ENCList = new List<WeakReference>();

  [DebuggerNonUserCode]
  static CReportBillFullPage21head()
  {
  }
  ...
  {
    lock (CReportBillFullPage21head.__ENCList)
    {
      if (CReportBillFullPage21head.__ENCList.Count == CReportBillFullPage21head.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (CReportBillFullPage21head.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (CReportBillFullPage21head.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              CReportBillFullPage21head.__ENCList[index1] = CReportBillFullPage21head.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        CReportBillFullPage21head.__ENCList.RemoveRange(index1, checked (CReportBillFullPage21head.__ENCList.Count - index1));
        CReportBillFullPage21head.__ENCList.Capacity = CReportBillFullPage21head.__ENCList.Count;
      }
      CReportBillFullPage21head.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }

  public CReportBillFullPage21head() => CReportBillFullPage21head.__ENCAddToList((object) this);

  public override string ResourceName
  {
    get => "CReportBillFullPage21head.rpt";
    set
    {
    }
  }
  ...
  {
    get => "MauliModule7.CReportBillFullPage21head.rpt";
    set
    {
    }
  }
