...
// Type: MauliModule7.CReportBillFullPageWithReceipt14Head
...
public class CReportBillFullPageWithReceipt14Head : ReportClass
{
  private static List<WeakReference> __ENCList = new List<WeakReference>();

  [DebuggerNonUserCode]
  static CReportBillFullPageWithReceipt14Head()
  {
  }
  ...
  {
    lock (CReportBillFullPageWithReceipt14Head.__ENCList)
    {
      if (CReportBillFullPageWithReceipt14Head.__ENCList.Count == CReportBillFullPageWithReceipt14Head.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (CReportBillFullPageWithReceipt14Head.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (CReportBillFullPageWithReceipt14Head.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              CReportBillFullPageWithReceipt14Head.__ENCList[index1] = CReportBillFullPageWithReceipt14Head.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        CReportBillFullPageWithReceipt14Head.__ENCList.RemoveRange(index1, checked (CReportBillFullPageWithReceipt14Head.__ENCList.Count - index1));
        CReportBillFullPageWithReceipt14Head.__ENCList.Capacity = CReportBillFullPageWithReceipt14Head.__ENCList.Count;
      }
      CReportBillFullPageWithReceipt14Head.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }

  public CReportBillFullPageWithReceipt14Head()
  {
    CReportBillFullPageWithReceipt14Head.__ENCAddToList((object) this);
  }

  public override string ResourceName
  {
    get => "CReportBillFullPageWithReceipt14Head.rpt";
    set
    {
    }
  }
  ...
  {
    get => "MauliModule7.CReportBillFullPageWithReceipt14Head.rpt";
    set
    {
    }
  }
