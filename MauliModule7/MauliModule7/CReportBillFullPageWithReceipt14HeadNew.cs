...
// Type: MauliModule7.CReportBillFullPageWithReceipt14HeadNew
...
public class CReportBillFullPageWithReceipt14HeadNew : ReportClass
{
  private static List<WeakReference> __ENCList = new List<WeakReference>();

  [DebuggerNonUserCode]
  static CReportBillFullPageWithReceipt14HeadNew()
  {
  }
  ...
  {
    lock (CReportBillFullPageWithReceipt14HeadNew.__ENCList)
    {
      if (CReportBillFullPageWithReceipt14HeadNew.__ENCList.Count == CReportBillFullPageWithReceipt14HeadNew.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (CReportBillFullPageWithReceipt14HeadNew.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (CReportBillFullPageWithReceipt14HeadNew.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              CReportBillFullPageWithReceipt14HeadNew.__ENCList[index1] = CReportBillFullPageWithReceipt14HeadNew.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        CReportBillFullPageWithReceipt14HeadNew.__ENCList.RemoveRange(index1, checked (CReportBillFullPageWithReceipt14HeadNew.__ENCList.Count - index1));
        CReportBillFullPageWithReceipt14HeadNew.__ENCList.Capacity = CReportBillFullPageWithReceipt14HeadNew.__ENCList.Count;
      }
      CReportBillFullPageWithReceipt14HeadNew.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }

  public CReportBillFullPageWithReceipt14HeadNew()
  {
    CReportBillFullPageWithReceipt14HeadNew.__ENCAddToList((object) this);
  }

  public override string ResourceName
  {
    get => "CReportBillFullPageWithReceipt14HeadNew.rpt";
    set
    {
    }
  }
  ...
  {
    get => "MauliModule7.CReportBillFullPageWithReceipt14HeadNew.rpt";
    set
    {
    }
  }
