...
// Type: MauliModule7.frmDigAccountSelection
...
[DesignerGenerated]
public class frmDigAccountSelection : Form
{
  ...
  [DebuggerNonUserCode]
  static frmDigAccountSelection()
  {
  }

  [DebuggerNonUserCode]
  public frmDigAccountSelection()
  {
    this.Load += new EventHandler(this.frmDigAccountSelection_Load);
    frmDigAccountSelection.__ENCAddToList((object) this);
  ...
  {
    lock (frmDigAccountSelection.__ENCList)
    {
      if (frmDigAccountSelection.__ENCList.Count == frmDigAccountSelection.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (frmDigAccountSelection.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (frmDigAccountSelection.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              frmDigAccountSelection.__ENCList[index1] = frmDigAccountSelection.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        frmDigAccountSelection.__ENCList.RemoveRange(index1, checked (frmDigAccountSelection.__ENCList.Count - index1));
        frmDigAccountSelection.__ENCList.Capacity = frmDigAccountSelection.__ENCList.Count;
      }
      frmDigAccountSelection.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }
  ...
  private void InitializeComponent()
  {
    ...
    Point point1 = new Point(323, 132);
    Point point2 = point1;
    ...
    this.Name = nameof (frmDigAccountSelection);
    this.ShowInTaskbar = false;
    this.StartPosition = FormStartPosition.CenterParent;
    this.Text = "Account Code Range";
  ...
  private void btnListFrom_Click(object sender, EventArgs e)
  {
    ...
    {
      this.lblMemFrom.Text = Conversions.ToString(MyProject.Forms.frmAccount.DataGridView2.CurrentRow.Cells[1].Value);
      this.lblMemNameFrom.Text = Conversions.ToString(MyProject.Forms.frmAccount.DataGridView2.CurrentRow.Cells[2].Value);
    }
  ...
  private void btnListTo_Click(object sender, EventArgs e)
  {
    ...
    if (Constant.SearchStat & MyProject.Forms.frmAccount.DataGridView2.SelectedRows.Count == 1)
    {
      this.lblMemTo.Text = Conversions.ToString(MyProject.Forms.frmAccount.DataGridView2.CurrentRow.Cells[1].Value);
      this.lblMemNameTo.Text = Conversions.ToString(MyProject.Forms.frmAccount.DataGridView2.CurrentRow.Cells[2].Value);
    }
    Constant.SearchStat = false;
    Constant.SearchStr = "";
  }

  private void frmDigAccountSelection_Load(object sender, EventArgs e)
  {
