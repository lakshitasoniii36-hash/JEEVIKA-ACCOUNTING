...
// Type: MauliModule7.CReportMemberInterest
...
public class CReportMemberInterest : ReportClass
{
  private static List<WeakReference> __ENCList = new List<WeakReference>();

  [DebuggerNonUserCode]
  static CReportMemberInterest()
  {
  }
  ...
  {
    lock (CReportMemberInterest.__ENCList)
    {
      if (CReportMemberInterest.__ENCList.Count == CReportMemberInterest.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (CReportMemberInterest.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (CReportMemberInterest.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              CReportMemberInterest.__ENCList[index1] = CReportMemberInterest.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        CReportMemberInterest.__ENCList.RemoveRange(index1, checked (CReportMemberInterest.__ENCList.Count - index1));
        CReportMemberInterest.__ENCList.Capacity = CReportMemberInterest.__ENCList.Count;
      }
      CReportMemberInterest.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }

  public CReportMemberInterest() => CReportMemberInterest.__ENCAddToList((object) this);

  public override string ResourceName
  {
    get => "CReportMemberInterest.rpt";
    set
    {
    }
  }
  ...
  {
    get => "MauliModule7.CReportMemberInterest.rpt";
    set
    {
    }
  }
