...
// Type: MauliModule7.SocietyBfDialog
...
[DesignerGenerated]
public class SocietyBfDialog : Form
{
  ...
  [DebuggerNonUserCode]
  static SocietyBfDialog()
  {
  }

  [DebuggerNonUserCode]
  public SocietyBfDialog()
  {
    this.Load += new EventHandler(this.DeleteDialog_Load);
    SocietyBfDialog.__ENCAddToList((object) this);
  ...
  {
    lock (SocietyBfDialog.__ENCList)
    {
      if (SocietyBfDialog.__ENCList.Count == SocietyBfDialog.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (SocietyBfDialog.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (SocietyBfDialog.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              SocietyBfDialog.__ENCList[index1] = SocietyBfDialog.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        SocietyBfDialog.__ENCList.RemoveRange(index1, checked (SocietyBfDialog.__ENCList.Count - index1));
        SocietyBfDialog.__ENCList.Capacity = SocietyBfDialog.__ENCList.Count;
      }
      SocietyBfDialog.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }
  ...
  private void InitializeComponent()
  {
    ...
    this.Label3.Text = "This Option Change balances";
    ...
    this.Name = nameof (SocietyBfDialog);
