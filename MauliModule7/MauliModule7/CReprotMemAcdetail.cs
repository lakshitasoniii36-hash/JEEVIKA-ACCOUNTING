...
// Type: MauliModule7.CReprotMemAcdetail
...
public class CReprotMemAcdetail : ReportClass
{
  private static List<WeakReference> __ENCList = new List<WeakReference>();

  [DebuggerNonUserCode]
  static CReprotMemAcdetail()
  {
  }
  ...
  {
    lock (CReprotMemAcdetail.__ENCList)
    {
      if (CReprotMemAcdetail.__ENCList.Count == CReprotMemAcdetail.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (CReprotMemAcdetail.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (CReprotMemAcdetail.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              CReprotMemAcdetail.__ENCList[index1] = CReprotMemAcdetail.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        CReprotMemAcdetail.__ENCList.RemoveRange(index1, checked (CReprotMemAcdetail.__ENCList.Count - index1));
        CReprotMemAcdetail.__ENCList.Capacity = CReprotMemAcdetail.__ENCList.Count;
      }
      CReprotMemAcdetail.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }

  public CReprotMemAcdetail() => CReprotMemAcdetail.__ENCAddToList((object) this);

  public override string ResourceName
  {
    get => "CReprotMemAcdetail.rpt";
    set
    {
    }
  }
  ...
  {
    get => "MauliModule7.CReprotMemAcdetail.rpt";
    set
    {
    }
  }
