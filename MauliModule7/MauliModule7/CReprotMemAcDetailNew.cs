...
// Type: MauliModule7.CReprotMemAcDetailNew
...
public class CReprotMemAcDetailNew : ReportClass
{
  private static List<WeakReference> __ENCList = new List<WeakReference>();

  [DebuggerNonUserCode]
  static CReprotMemAcDetailNew()
  {
  }
  ...
  {
    lock (CReprotMemAcDetailNew.__ENCList)
    {
      if (CReprotMemAcDetailNew.__ENCList.Count == CReprotMemAcDetailNew.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (CReprotMemAcDetailNew.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (CReprotMemAcDetailNew.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              CReprotMemAcDetailNew.__ENCList[index1] = CReprotMemAcDetailNew.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        CReprotMemAcDetailNew.__ENCList.RemoveRange(index1, checked (CReprotMemAcDetailNew.__ENCList.Count - index1));
        CReprotMemAcDetailNew.__ENCList.Capacity = CReprotMemAcDetailNew.__ENCList.Count;
      }
      CReprotMemAcDetailNew.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }

  public CReprotMemAcDetailNew() => CReprotMemAcDetailNew.__ENCAddToList((object) this);

  public override string ResourceName
  {
    get => "CReprotMemAcDetailNew.rpt";
    set
    {
    }
  }
  ...
  {
    get => "MauliModule7.CReprotMemAcDetailNew.rpt";
    set
    {
    }
  }
