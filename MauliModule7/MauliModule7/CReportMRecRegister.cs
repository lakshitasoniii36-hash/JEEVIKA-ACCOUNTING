...
// Type: MauliModule7.CReportMRecRegister
...
public class CReportMRecRegister : ReportClass
{
  private static List<WeakReference> __ENCList = new List<WeakReference>();

  [DebuggerNonUserCode]
  static CReportMRecRegister()
  {
  }
  ...
  {
    lock (CReportMRecRegister.__ENCList)
    {
      if (CReportMRecRegister.__ENCList.Count == CReportMRecRegister.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (CReportMRecRegister.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (CReportMRecRegister.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              CReportMRecRegister.__ENCList[index1] = CReportMRecRegister.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        CReportMRecRegister.__ENCList.RemoveRange(index1, checked (CReportMRecRegister.__ENCList.Count - index1));
        CReportMRecRegister.__ENCList.Capacity = CReportMRecRegister.__ENCList.Count;
      }
      CReportMRecRegister.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }

  public CReportMRecRegister() => CReportMRecRegister.__ENCAddToList((object) this);

  public override string ResourceName
  {
    get => "CReportMRecRegister.rpt";
    set
    {
    }
  }
  ...
  {
    get => "MauliModule7.CReportMRecRegister.rpt";
    set
    {
    }
  }
