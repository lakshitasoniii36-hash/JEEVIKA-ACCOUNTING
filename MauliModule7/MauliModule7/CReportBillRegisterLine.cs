...
// Type: MauliModule7.CReportBillRegisterLine
...
public class CReportBillRegisterLine : ReportClass
{
  private static List<WeakReference> __ENCList = new List<WeakReference>();

  [DebuggerNonUserCode]
  static CReportBillRegisterLine()
  {
  }
  ...
  {
    lock (CReportBillRegisterLine.__ENCList)
    {
      if (CReportBillRegisterLine.__ENCList.Count == CReportBillRegisterLine.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (CReportBillRegisterLine.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (CReportBillRegisterLine.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              CReportBillRegisterLine.__ENCList[index1] = CReportBillRegisterLine.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        CReportBillRegisterLine.__ENCList.RemoveRange(index1, checked (CReportBillRegisterLine.__ENCList.Count - index1));
        CReportBillRegisterLine.__ENCList.Capacity = CReportBillRegisterLine.__ENCList.Count;
      }
      CReportBillRegisterLine.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }

  public CReportBillRegisterLine() => CReportBillRegisterLine.__ENCAddToList((object) this);

  public override string ResourceName
  {
    get => "CReportBillRegisterLine.rpt";
    set
    {
    }
  }
  ...
  {
    get => "MauliModule7.CReportBillRegisterLine.rpt";
    set
    {
    }
  }
