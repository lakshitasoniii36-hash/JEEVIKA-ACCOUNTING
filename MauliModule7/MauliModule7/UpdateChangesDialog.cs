...
// Type: MauliModule7.UpdateChangesDialog
...
[DesignerGenerated]
public class UpdateChangesDialog : Form
{
  ...
  [DebuggerNonUserCode]
  static UpdateChangesDialog()
  {
  }

  [DebuggerNonUserCode]
  public UpdateChangesDialog()
  {
    this.Load += new EventHandler(this.DeleteDialog_Load);
    UpdateChangesDialog.__ENCAddToList((object) this);
  ...
  {
    lock (UpdateChangesDialog.__ENCList)
    {
      if (UpdateChangesDialog.__ENCList.Count == UpdateChangesDialog.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (UpdateChangesDialog.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (UpdateChangesDialog.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              UpdateChangesDialog.__ENCList[index1] = UpdateChangesDialog.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        UpdateChangesDialog.__ENCList.RemoveRange(index1, checked (UpdateChangesDialog.__ENCList.Count - index1));
        UpdateChangesDialog.__ENCList.Capacity = UpdateChangesDialog.__ENCList.Count;
      }
      UpdateChangesDialog.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }
  ...
  private void InitializeComponent()
  {
    ...
    this.Name = nameof (UpdateChangesDialog);
  ...
  private void DeleteDialog_Load(object sender, EventArgs e) => this.lblResult.Text = "1";
}
