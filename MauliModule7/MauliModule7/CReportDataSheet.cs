...
// Type: MauliModule7.CReportDataSheet
...
public class CReportDataSheet : ReportClass
{
  private static List<WeakReference> __ENCList = new List<WeakReference>();

  [DebuggerNonUserCode]
  static CReportDataSheet()
  {
  }
  ...
  {
    lock (CReportDataSheet.__ENCList)
    {
      if (CReportDataSheet.__ENCList.Count == CReportDataSheet.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (CReportDataSheet.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (CReportDataSheet.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              CReportDataSheet.__ENCList[index1] = CReportDataSheet.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        CReportDataSheet.__ENCList.RemoveRange(index1, checked (CReportDataSheet.__ENCList.Count - index1));
        CReportDataSheet.__ENCList.Capacity = CReportDataSheet.__ENCList.Count;
      }
      CReportDataSheet.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }

  public CReportDataSheet() => CReportDataSheet.__ENCAddToList((object) this);

  public override string ResourceName
  {
    get => "CReportDataSheet.rpt";
    set
    {
    }
  }
  ...
  {
    get => "MauliModule7.CReportDataSheet.rpt";
    set
    {
    }
  }
  ...
  [Browsable(false)]
  [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
  public Section ReportFooterSection3 => this.ReportDefinition.Sections[5];
