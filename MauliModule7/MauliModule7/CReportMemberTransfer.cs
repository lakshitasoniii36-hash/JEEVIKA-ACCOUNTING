...
// Type: MauliModule7.CReportMemberTransfer
...
public class CReportMemberTransfer : ReportClass
{
  private static List<WeakReference> __ENCList = new List<WeakReference>();

  [DebuggerNonUserCode]
  static CReportMemberTransfer()
  {
  }
  ...
  {
    lock (CReportMemberTransfer.__ENCList)
    {
      if (CReportMemberTransfer.__ENCList.Count == CReportMemberTransfer.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (CReportMemberTransfer.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (CReportMemberTransfer.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              CReportMemberTransfer.__ENCList[index1] = CReportMemberTransfer.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        CReportMemberTransfer.__ENCList.RemoveRange(index1, checked (CReportMemberTransfer.__ENCList.Count - index1));
        CReportMemberTransfer.__ENCList.Capacity = CReportMemberTransfer.__ENCList.Count;
      }
      CReportMemberTransfer.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }

  public CReportMemberTransfer() => CReportMemberTransfer.__ENCAddToList((object) this);

  public override string ResourceName
  {
    get => "CReportMemberTransfer.rpt";
    set
    {
    }
  }
  ...
  {
    get => "MauliModule7.CReportMemberTransfer.rpt";
    set
    {
    }
  }
