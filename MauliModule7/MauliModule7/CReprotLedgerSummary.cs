...
// Type: MauliModule7.CReprotLedgerSummary
...
public class CReprotLedgerSummary : ReportClass
{
  private static List<WeakReference> __ENCList = new List<WeakReference>();

  [DebuggerNonUserCode]
  static CReprotLedgerSummary()
  {
  }
  ...
  {
    lock (CReprotLedgerSummary.__ENCList)
    {
      if (CReprotLedgerSummary.__ENCList.Count == CReprotLedgerSummary.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (CReprotLedgerSummary.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (CReprotLedgerSummary.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              CReprotLedgerSummary.__ENCList[index1] = CReprotLedgerSummary.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        CReprotLedgerSummary.__ENCList.RemoveRange(index1, checked (CReprotLedgerSummary.__ENCList.Count - index1));
        CReprotLedgerSummary.__ENCList.Capacity = CReprotLedgerSummary.__ENCList.Count;
      }
      CReprotLedgerSummary.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }

  public CReprotLedgerSummary() => CReprotLedgerSummary.__ENCAddToList((object) this);

  public override string ResourceName
  {
    get => "CReprotLedgerSummary.rpt";
    set
    {
    }
  }
  ...
  {
    get => "MauliModule7.CReprotLedgerSummary.rpt";
    set
    {
    }
  }
  ...
  [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
  [Browsable(false)]
  public Section GroupFooterSection3 => this.ReportDefinition.Sections[5];
