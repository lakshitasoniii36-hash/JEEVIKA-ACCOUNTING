...
// Type: MauliModule7.OverwirteDialog
...
[DesignerGenerated]
public class OverwirteDialog : Form
{
  ...
  [DebuggerNonUserCode]
  static OverwirteDialog()
  {
  }

  [DebuggerNonUserCode]
  public OverwirteDialog()
  {
    this.Load += new EventHandler(this.DeleteDialog_Load);
    OverwirteDialog.__ENCAddToList((object) this);
  ...
  {
    lock (OverwirteDialog.__ENCList)
    {
      if (OverwirteDialog.__ENCList.Count == OverwirteDialog.__ENCList.Capacity)
      {
        int index1 = 0;
        int num = checked (OverwirteDialog.__ENCList.Count - 1);
        int index2 = 0;
        while (index2 <= num)
        {
          if (OverwirteDialog.__ENCList[index2].IsAlive)
          {
            if (index2 != index1)
              OverwirteDialog.__ENCList[index1] = OverwirteDialog.__ENCList[index2];
            checked { ++index1; }
          }
          checked { ++index2; }
        }
        OverwirteDialog.__ENCList.RemoveRange(index1, checked (OverwirteDialog.__ENCList.Count - index1));
        OverwirteDialog.__ENCList.Capacity = OverwirteDialog.__ENCList.Count;
      }
      OverwirteDialog.__ENCList.Add(new WeakReference(RuntimeHelpers.GetObjectValue(value)));
    }
  }
  ...
  private void InitializeComponent()
  {
    ...
    this.Label1.Text = "File Exist Overwrite";
    ...
    this.Name = nameof (OverwirteDialog);
  ...
  private void DeleteDialog_Load(object sender, EventArgs e) => this.lblResult.Text = "0";
}
