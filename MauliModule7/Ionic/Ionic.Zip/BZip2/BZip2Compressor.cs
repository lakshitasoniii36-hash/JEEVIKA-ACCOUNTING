...
internal class BZip2Compressor
{
  ...
  private int nInUse;
  private int nMTF;
  private static readonly int SETMASK = 2097152 /*0x200000*/;
  ...
  private void mainSort()
  {
    ...
    for (int index5 = 1; index5 <= 65536 /*0x010000*/; ++index5)
              ...
              if ((int) block[num7 + 2] == (int) block[num8 + 2])
              {
                if ((int) block[num7 + 3] == (int) block[num8 + 3])
                {
                  if ((int) block[num7 + 4] == (int) block[num8 + 4])
                  {
                    ...
                    if (((int) block[num7 + 5] & (int) byte.MaxValue) <= ((int) block[num8 + 5] & (int) byte.MaxValue))
                      break;
                    continue;
                  }
                  if (((int) block[num7 + 4] & (int) byte.MaxValue) <= ((int) block[num8 + 4] & (int) byte.MaxValue))
                    break;
                  continue;
                }
                if (((int) block[num7 + 3] & (int) byte.MaxValue) <= ((int) block[num8 + 3] & (int) byte.MaxValue))
                  break;
                continue;
              }
              if (((int) block[num7 + 2] & (int) byte.MaxValue) <= ((int) block[num8 + 2] & (int) byte.MaxValue))
                break;
              continue;
            }
            continue;
...
label_54:
  ...
  private void sendMTFValues4()
  {
    ...
    for (int index2 = 0; index2 < 16 /*0x10*/; ++index2)
    {
      if (mtfValues4InUse16[index2])
        num3 |= (uint) (1 << 16 /*0x10*/ - index2 - 1);
    }
    this.bw.WriteBits(16 /*0x10*/, num3);
    for (int index3 = 0; index3 < 16 /*0x10*/; ++index3)
    {
      if (mtfValues4InUse16[index3])
      {
        int num4 = index3 * 16 /*0x10*/;
        uint num5 = 0;
        for (int index4 = 0; index4 < 16 /*0x10*/; ++index4)
        {
          if (inUse[num4 + index4])
            num5 |= (uint) (1 << 16 /*0x10*/ - index4 - 1);
        }
        this.bw.WriteBits(16 /*0x10*/, num5);
      }
    }
  }
  ...
  private class CompressionState
  {
    ...
    public readonly byte[] unseqToSeq = new byte[256 /*0x0100*/];
    ...
    public readonly byte[] generateMTFValues_yy = new byte[256 /*0x0100*/];
    ...
    public readonly bool[] sentMTFValues4_inUse16 = new bool[16 /*0x10*/];
    ...
    public readonly int[] mainSort_runningOrder = new int[256 /*0x0100*/];
    public readonly int[] mainSort_copy = new int[256 /*0x0100*/];
    public readonly bool[] mainSort_bigDone = new bool[256 /*0x0100*/];
    ...
    public readonly int[] ftab = new int[65537 /*0x010001*/];
    public byte[] block;
    public int[] fmap;
    public char[] sfmap;
