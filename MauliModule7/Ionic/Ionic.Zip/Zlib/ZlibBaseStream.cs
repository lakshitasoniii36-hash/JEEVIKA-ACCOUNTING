...
internal class ZlibBaseStream : Stream
{
  ...
  protected internal int _bufferSize = 16384 /*0x4000*/;
  ...
  private void finish()
  {
    ...
    else
    {
      ...
      if (this._z.AvailableBytesIn < 8)
      {
        ...
        if (count != num)
          throw new ZlibException($"Missing or incomplete GZIP trailer. Expected 8 bytes, got {this._z.AvailableBytesIn + num}.");
      }
      else
      ...
      if (crc32Result != int32_1)
        throw new ZlibException($"Bad CRC32 in GZIP trailer. (actual({crc32Result:X8})!=expected({int32_1:X8}))");
      if (num1 != int32_2)
        throw new ZlibException($"Bad size in GZIP trailer. (actual({num1})!=expected({int32_2}))");
    }
  }
  ...
  private int _ReadAndValidateGzipHeader()
  {
    ...
    switch (num2)
    {
      case 0:
        return 0;
      case 10:
        int num3 = buffer1[0] == (byte) 31 /*0x1F*/ && buffer1[1] == (byte) 139 && buffer1[2] == (byte) 8 ? BitConverter.ToInt32(buffer1, 4) : throw new ZlibException("Bad GZIP header.");
        ...
        if (((int) buffer1[3] & 4) == 4)
        {
          ...
          int num6 = num4 + num5;
          short length = (short) ((int) buffer1[0] + (int) buffer1[1] * 256 /*0x0100*/);
        ...
        if (((int) buffer1[3] & 16 /*0x10*/) == 16 /*0x10*/)
  ...
  public override int Read(byte[] buffer, int offset, int count)
  {
    ...
    do
    {
      ...
      if (this.nomoreinput && num1 == -5)
        return 0;
      if (num1 != 0 && num1 != 1)
        throw new ZlibException($"{(this._wantCompress ? (object) "de" : (object) "in")}flating:  rc={num1}  msg={this._z.Message}");
    }
      ...
      if (this.nomoreinput && this._wantCompress)
      {
        ...
        switch (num2)
        {
          ...
          case 1:
            break;
          default:
            throw new ZlibException($"Deflating:  rc={num2}  msg={this._z.Message}");
        }
      }
    }
