...
public class CRC32
{
  ...
  {
    return (int) this.crc32Table[(IntPtr) (uint) (((int) W ^ (int) B) & (int) byte.MaxValue)] ^ (int) (W >> 8);
  }
  ...
  public void UpdateCRC(byte b, int n)
  {
    while (n-- > 0)
    {
      if (this.reverseBits)
      {
        ...
        this._register = this._register << 8 ^ this.crc32Table[num >= 0U ? (IntPtr) num : (IntPtr) (num + 256U /*0x0100*/)];
      }
      else
      {
  ...
  private static byte ReverseBits(byte data)
  {
    ...
    uint num2 = 17055760;
    return (byte) (16781313 /*0x01001001*/ * ((int) (num1 & num2) + ((int) num1 << 2 & (int) num2 << 1)) >>> 24);
  }
