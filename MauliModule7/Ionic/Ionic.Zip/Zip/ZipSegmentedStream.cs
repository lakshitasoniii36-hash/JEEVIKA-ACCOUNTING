...
internal class ZipSegmentedStream : Stream
{
  ...
  {
    return diskNumber < 99U ? (Stream) File.Open($"{Path.Combine(Path.GetDirectoryName(name), Path.GetFileNameWithoutExtension(name))}.z{(ValueType) (uint) ((int) diskNumber + 1):D2}", FileMode.Open, FileAccess.ReadWrite, FileShare.None) : throw new ArgumentOutOfRangeException(nameof (diskNumber));
  }
  ...
  {
    return this._innerStream.Position + (long) length > (long) this._maxSegmentSize ? this.CurrentSegment + 1U : this.CurrentSegment;
  }
