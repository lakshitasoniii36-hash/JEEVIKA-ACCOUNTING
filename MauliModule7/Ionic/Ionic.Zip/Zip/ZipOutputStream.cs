...
public class ZipOutputStream : Stream
{
  ...
  {
    this._Init((Stream) File.Open(fileName, FileMode.Create, FileAccess.ReadWrite, FileShare.None), false, fileName);
  }
  ...
  {
    return $"ZipOutputStream::{this._name}(leaveOpen({this._leaveUnderlyingStreamOpen})))";
  }
  ...
  public long ParallelDeflateThreshold
  {
    set
    {
      this._ParallelDeflateThreshold = value == 0L || value == -1L || value >= 65536L /*0x010000*/ ? value : throw new ArgumentOutOfRangeException("value must be greater than 64k, or 0, or -1");
    }
