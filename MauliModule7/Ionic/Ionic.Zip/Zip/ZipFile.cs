...
public class ZipFile : IEnumerable<ZipEntry>, IEnumerable, IDisposable
{
  ...
  private int _maxBufferPairs = 16 /*0x10*/;
  internal Zip64Option _zip64;
  private bool _SavingSfx;
  public static readonly int BufferSizeDefault = 32768 /*0x8000*/;
  ...
  public ZipEntry AddItem(string fileOrDirectoryName, string directoryPathInArchive)
  {
    ...
    return Directory.Exists(fileOrDirectoryName) ? this.AddDirectory(fileOrDirectoryName, directoryPathInArchive) : throw new FileNotFoundException($"That file or directory ({fileOrDirectoryName}) does not exist!");
  }
  ...
  public static bool CheckZip(string zipFileName, bool fixIfNecessary, TextWriter writer)
  {
    ...
    try
    {
      ...
      if (!flag)
      {
        if (fixIfNecessary)
        {
          string fileName = $"{Path.GetFileNameWithoutExtension(zipFileName)}_fixed.zip";
  ...
  public string Info
  {
    get
    {
      ...
      if (this._versionMadeBy != (ushort) 0)
        stringBuilder.Append($"  version made by: 0x{this._versionMadeBy:X4}\n");
      if (this._versionNeededToExtract != (ushort) 0)
        stringBuilder.Append($"needed to extract: 0x{this._versionNeededToExtract:X4}\n");
      stringBuilder.Append($"       uses ZIP64: {this.InputUsesZip64}\n");
      stringBuilder.Append($"     disk with CD: {this._diskNumberWithCd}\n");
      if (this._OffsetOfCentralDirectory == uint.MaxValue)
        stringBuilder.Append($"      CD64 offset: 0x{this._OffsetOfCentralDirectory64:X16}\n");
      else
        stringBuilder.Append($"        CD offset: 0x{this._OffsetOfCentralDirectory:X8}\n");
  ...
  public Encoding ProvisionalAlternateEncoding
  {
    get
    {
      return this._alternateEncodingUsage == ZipOption.AsNecessary ? this._alternateEncoding : (Encoding) null;
    }
    set
    {
  ...
  public int MaxOutputSegmentSize
  {
    ...
    set
    {
      this._maxOutputSegmentSize = value >= 65536 /*0x010000*/ || value == 0 ? value : throw new ZipException("The minimum acceptable segment size is 65536.");
    }
  }
  ...
  public long ParallelDeflateThreshold
  {
    set
    {
      this._ParallelDeflateThreshold = value == 0L || value == -1L || value >= 65536L /*0x010000*/ ? value : throw new ArgumentOutOfRangeException("ParallelDeflateThreshold should be -1, 0, or > 65536");
    }
  ...
  public int ParallelDeflateMaxBufferPairs
  {
    ...
    set
    {
      this._maxBufferPairs = value >= 4 ? value : throw new ArgumentOutOfRangeException(nameof (ParallelDeflateMaxBufferPairs), "Value must be 4 or greater.");
    }
  }

  public override string ToString() => $"ZipFile::{this.Name}";
  ...
  {
    return (int) diskNumber + 1 == (int) this._diskNumberWithCd || diskNumber == 0U && this._diskNumberWithCd == 0U ? this.ReadStream : (Stream) ZipSegmentedStream.ForReading(this._readName ?? this._name, diskNumber, this._diskNumberWithCd);
  }
  ...
  public ZipFile(string fileName)
  {
    ...
    {
      throw new ZipException($"Could not read {fileName} as a zip file", ex);
    }
  }
  ...
  public ZipFile(string fileName, Encoding encoding)
  {
    ...
    {
      throw new ZipException($"{fileName} is not a valid zip file", ex);
    }
  }
  ...
  private void _initEntriesDictionary()
  {
    ...
    this._entries = this._entries == null ? new Dictionary<string, ZipEntry>((IEqualityComparer<string>) comparer) : new Dictionary<string, ZipEntry>((IDictionary<string, ZipEntry>) this._entries, (IEqualityComparer<string>) comparer);
  }
  ...
  private void _InitInstance(string zipFileName, TextWriter statusMessageWriter)
  {
    ...
    this.ParallelDeflateThreshold = 524288L /*0x080000*/;
    this._initEntriesDictionary();
  ...
  {
    this.RemoveEntry(this[ZipEntry.NameInArchive(fileName, (string) null)] ?? throw new ArgumentException("The entry you specified was not found in the zip archive."));
  }
  ...
  private string ArchiveNameForEvent => this._name == null ? "(stream)" : this._name;
  ...
  private static void ReadIntoInstance(ZipFile zf)
  {
    ...
    try
    {
      ...
      int num1 = 0;
      bool flag = false;
      long offset = readStream.Length - 64L /*0x40*/;
      long num2 = Math.Max(readStream.Length - 16384L /*0x4000*/, 10L);
      do
      {
        if (offset < 0L)
          offset = 0L;
        ...
        {
          ++num1;
          offset -= (long) (32 /*0x20*/ * (num1 + 1) * num1);
        }
        else
          break;
      }
  ...
  private static void Zip64SeekToCentralDirectory(ZipFile zf)
  {
    ...
    readStream.Read(buffer1, 0, 16 /*0x10*/);
    ...
    if (num != 101075792U)
      throw new BadReadException($"  Bad signature (0x{num:X8}) looking for ZIP64 EoCD Record at position 0x{readStream.Position:X8}");
  ...
  private static void ReadCentralDirectoryFooter(ZipFile zf)
  {
    ...
    if (num1 != 101010256)
    {
      ...
      throw new BadReadException($"Bad signature ({num1:X8}) at position 0x{readStream.Position:X8}");
    }
    byte[] buffer = new byte[16 /*0x10*/];
  ...
  private static void ReadZipFileComment(ZipFile zf)
  {
    ...
    zf.ReadStream.Read(buffer, 0, buffer.Length);
    short length = (short) ((int) buffer[0] + (int) buffer[1] * 256 /*0x0100*/);
    if (length <= (short) 0)
      return;
  ...
  public void Save()
  {
    try
    {
      ...
      else
      {
        ...
        if (this._saveOperationCanceled)
          return;
        this._numberOfSegmentsForMostRecentSave = this.WriteStream is ZipSegmentedStream writeStream ? writeStream.CurrentSegment : 1U;
        ...
        if (this._name != null && (this._temporaryFileName != null || writeStream != null))
        {
          ...
          if (this._fileAlreadyExists && this._readstream != null)
          {
            ...
            {
              if (zipEntry._archiveStream is ZipSegmentedStream archiveStream)
    ...
    string replacement)
  {
    ...
    return upper1.IndexOf(upper2) != 0 ? original : replacement + original.Substring(upper2.Length);
  }
      ...
      using (CSharpCodeProvider csharpCodeProvider = new CSharpCodeProvider())
      {
        ...
        using (ZipFile zipFile = ZipFile.Read(executingAssembly.GetManifestResourceStream("Ionic.Zip.Resources.ZippedResources.zip")))
        {
          ...
          if (string.IsNullOrEmpty(options.IconFile))
          {
            ...
            options1.CompilerOptions += $"/win32icon:\"{str2}\"";
          }
          else
            options1.CompilerOptions += $"/win32icon:\"{options.IconFile}\"";
          ...
          stringBuilder.Append($"[assembly: System.Reflection.AssemblyCopyright(\"{str3}\")]\n").Append($"[assembly: System.Reflection.AssemblyVersion(\"{ZipFile.LibraryVersion.ToString()}\")]\n");
          ...
          foreach (string fileName in extractorSettings.ResourcesToCompile)
          {
            ...
            {
              if (stream == null)
                throw new ZipException($"missing resource '{fileName}'");
        ...
        if (compilerResults.Errors.Count != 0)
        {
          using (TextWriter textWriter = (TextWriter) new StreamWriter(tempPathname))
          {
            ...
            foreach (CompilerError error in (CollectionBase) compilerResults.Errors)
              textWriter.Write($"//   {fileName}({error.Line},{error.Column}): {(error.IsWarning ? (object) "Warning" : (object) "error")} {error.ErrorNumber}: {error.ErrorText}\n//\n");
          }
          throw new SfxGenerationException($"Errors compiling the extraction logic!  {tempPathname}");
        }
  ...
  internal static string GenerateTempPathname(string dir, string @extension)
  {
    ...
    do
    {
      ...
      string path2 = $"{name}-{DateTime.Now.ToString("yyyyMMMdd-HHmmss")}-{str}.{@extension}";
