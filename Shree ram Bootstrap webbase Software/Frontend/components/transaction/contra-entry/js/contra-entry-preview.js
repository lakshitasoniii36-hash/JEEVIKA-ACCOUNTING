...
var ContraEntryPreview = (function () {
  ...
  function render() {
    ...
    html += '<div class="ce-invoice-society-name" style="font-size:22px; font-weight:800; letter-spacing:0.5px; text-transform:uppercase; color:#1565C0; margin-bottom:15px; text-align:center;">' + socName + '</div>';
    html += '<div class="ce-invoice-title-bar" style="font-size:16px; font-weight:bold; text-align:center; border-top:1.5px solid #000; border-bottom:1.5px solid #000; padding:4px 0; margin-bottom:15px; text-transform:uppercase;">CONTRA VOUCHER</div>';
    html += '</div>';
    ...
    html += '  <div><strong>Person Name:</strong> ' + (c.personName || '—') + '</div>';
    html += '  <div><strong>Voucher No:</strong> ' + (c.voucherNo || '—') + '</div>';
    html += '  <div><strong>Date:</strong> ' + window.formatDateToDDMMYYYY(c.voucherDate) + '</div>';
    html += '</div>';
    ...
    html += '  <div><strong>Cash/Bank (Cr.):</strong> ' + (c.cashBankCode || 'CASH') + ' - ' + (c.cashBankName || 'Cash in Hand') + '</div>';
    html += '  <div><strong>Trans Type:</strong> ' + (c.transType || 'Cheque') + '</div>';
    if (c.transType !== 'Cash') {
      html += '  <div><strong>Cheque/Ref No:</strong> ' + (c.chqNo || c.refNo || '—') + '</div>';
      if (c.chqDate) html += '  <div><strong>Cheque Date:</strong> ' + window.formatDateToDDMMYYYY(c.chqDate) + '</div>';
      if (c.drawnOn) html += '  <div><strong>Drawn On:</strong> ' + c.drawnOn + '</div>';
    }
    html += '</div>';

    // Items
    html += '<table class="ce-invoice-items-table" style="width:100%; border-collapse:collapse; font-family:\'Courier New\', monospace; font-size:13px; margin-bottom:20px;"><thead><tr style="border-top:1.5px solid #000; border-bottom:1.5px solid #000; font-weight:bold;">';
    ...
    // Remarks
    if (c.particular1 || c.particular2) {
      ...
      if (c.particular1) html += '  <div><strong>Narration 1:</strong> ' + c.particular1 + '</div>';
      if (c.particular2) html += '  <div><strong>Narration 2:</strong> ' + c.particular2 + '</div>';
      html += '</div>';
    }

    html += '<div class="ce-invoice-signatures" style="margin-top:50px;">';
