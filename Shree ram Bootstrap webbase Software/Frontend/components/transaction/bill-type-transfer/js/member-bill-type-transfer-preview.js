...
var MemberBillTypeTransferPreview = (function () {
  ...
  function render() {
    ...
    if (rawNarr.indexOf('|| METADATA:') > -1) {
      var parts = rawNarr.split('|| METADATA:');
      rawNarr = parts[0].trim();
      try {
    ...
    var m2Name = m2 ? m2.name : '—';
    var m2Flat = m2 ? m2.wingFlat : '—';
    ...
    html += '<tr><td class="mbtt-info-label">1st Member</td><td class="mbtt-info-value"><strong>' + t.memberName + ' (' + t.memberCode + ', Flat: ' + t.wingFlat + ')</strong></td></tr>';
    html += '<tr><td class="mbtt-info-label">2nd Member</td><td class="mbtt-info-value"><strong>' + m2Name + ' (' + code2 + ', Flat: ' + m2Flat + ')</strong></td></tr>';
    html += '</table></div>';
    ...
    var particulars = [];
    if (part1) particulars.push(part1);
    if (part2) particulars.push(part2);
    if (particulars.length === 0 && t.particulars && Array.isArray(t.particulars)) {
    ...
    if(particulars.length > 0 || chqNo || bank || transType !== 'Cash') {
      html += '<div style="margin-bottom:16px;font-size:11px;color:#424242;padding:8px;border:1px solid #E0E0E0;background:#FAFAFA;">';
      if (transType && transType !== 'Cash') {
        html += '<div style="margin-bottom:4px;"><strong>Transaction Type:</strong> ' + transType + '</div>';
      }
      if (particulars.length > 0) {
        html += '<div style="margin-bottom:4px;"><strong>Remarks:</strong> ' + particulars.join(' | ') + '</div>';
      }
      if(chqNo) html += '<div><strong>Cheque Details:</strong> Chq No: ' + chqNo + ' | Date: ' + (chqDate ? window.formatDateToDDMMYYYY(chqDate) : '-') + ' | Bank: ' + (bank||'-') + '</div>';
      if(refNo) html += '<div style="margin-top:4px;"><strong>Reference/UTR No:</strong> ' + refNo + '</div>';
      html += '</div>';
    }
