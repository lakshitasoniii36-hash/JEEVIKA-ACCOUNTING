...
async function handlePageNavigation(pageName) {
  ...
  try {
    const category = (typeof ModulesCategoryMap !== 'undefined' && ModulesCategoryMap[pageName]) ? ModulesCategoryMap[pageName] : 'master';
    const response = await fetch(`components/${category}/${pageName}/${pageName}.html`, { cache: 'no-cache' });
